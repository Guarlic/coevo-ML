...
def play_full_league(directory, strategies):
    ...
    # 1. 실제 함수 객체를 저장할 딕셔너리 생성
    callable_strategies = {}
    for module_name, func_name in strategies.items():
        # importlib을 사용해 모듈을 동적으로 임포트
        module = importlib.import_module(f"{directory}.{module_name}")
        # 임포트한 모듈에서 함수 객체를 이름으로 가져와 저장
        callable_strategies[func_name] = getattr(module, func_name)

    # 2. 함수 '이름'이 아닌 실제 '객체'로 쌍을 만듦
    pairs_of_strategies = []
    strategy_funcs = list(callable_strategies.values())
    for i in range(len(strategy_funcs)):
        for j in range(i, len(strategy_funcs)):
            pairs_of_strategies.append((strategy_funcs[i], strategy_funcs[j]))
    ...
    for pair_of_strategies in pairs_of_strategies:
        ...
        for i in range(n):
            # 3. eval 대신 함수를 직접 호출
            left_decision = left(left_decisions, right_decisions)
            right_decision = right(right_decisions, left_decisions)
        ...
        # total_records에는 함수 이름으로 기록 (make_report 호환성 유지)
        left_name = pair_of_strategies[0].__name__
        right_name = pair_of_strategies[1].__name__
        total_records[(left_name, right_name)] = (left_decisions, right_decisions)
